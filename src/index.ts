...
try {
  ...
  const uripath = URL.pathToFileURL(path.resolve('./')).toString() + '/'
  RecordSchema.fromDoc(doc, 'http://example.com', new LoadingOptions({ fileUri: uripath })).then((val) => console.log((val as RecordSchema).fields)).catch((e) => console.log((e as ValidationException).toString()))
} catch (e) {
  if (e instanceof ValidationException) {
    console.log(e.toString())
  }
