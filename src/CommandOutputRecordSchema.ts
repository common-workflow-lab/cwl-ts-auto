...
export class CommandOutputRecordSchema extends Saveable implements Internal.CommandOutputRecordSchemaProperties {
  ...
  static override async fromDoc (__doc: any, baseuri: string, loadingOptions: LoadingOptions,
    ...
    if ('fields' in _doc) {
      try {
        fields = await loadField(_doc.fields, LoaderInstances.idmapfieldsunionOfundefinedtypeOrarrayOfCommandOutputRecordFieldLoader,
          baseuri, loadingOptions)
      } catch (e) {
        if (e instanceof ValidationException) {
          __errors.push(
    ...
    if (__errors.length > 0) {
      throw new ValidationException("Trying 'CommandOutputRecordSchema'", __errors)
    }
